...
class DataQualityValidator:
    ...
    def validate_fixtures_data(self, df: pd.DataFrame, competition: str = "", season: str = "") -> Dict[str, Any]:
        """
    ...
    def _validate_goals(self, df: pd.DataFrame):
        ...
        for col in ['goals_home', 'goals_away']:
            ...
            if high_scores.any():
                ...
                self._add_warning(f"Found {count} unusually high scores in {col} (max: {max_score})")
            
            # Statistical checks
            if not numeric_goals.empty:
                mean_goals = numeric_goals.mean()
                if mean_goals < 0.5 or mean_goals > 5.0:
                    self._add_warning(f"Unusual average for {col}: {mean_goals:.2f} goals per game")
    ...
    def _validate_dates(self, df: pd.DataFrame, season: str = ""):
        ...
        self._add_info(f"Date range: {min_date.date()} to {max_date.date()} ({date_range.days} days)")
        ...
        if season and '-' in season:
            try:
                ...
                expected_start = datetime(start_year, 7, 1)  # Assume season starts in July
                expected_end = datetime(end_year, 6, 30)    # Ends in June
                
                outside_season = (valid_dates < expected_start) | (valid_dates > expected_end)
                if outside_season.any():
                    count = outside_season.sum()
                    self._add_warning(f"Found {count} dates outside expected season range")
            except ValueError:
    ...
    def _check_duplicates(self, df: pd.DataFrame, context: str = ""):
        ...
        if 'date' in df.columns:
            df_reversed = df.copy()
            df_reversed['team_home'], df_reversed['team_away'] = df['team_away'], df['team_home']
            
            # Find matches where teams play each other twice on same date
            merged = df.merge(df_reversed, on=['team_home', 'team_away', 'date'], how='inner')
            if not merged.empty:
                count = len(merged)
    ...
    def _validate_data_consistency(self, df: pd.DataFrame, context: str = ""):
        ...
        if not all(col in df.columns for col in ['team_home', 'team_away', 'goals_home', 'goals_away']):
            return
        ...
        if not goals_home.empty and not goals_away.empty:
            ...
            if always_home_wins:
                self._add_warning("All matches result in home wins - suspicious pattern")
            if always_away_wins:
                self._add_warning("All matches result in away wins - suspicious pattern")
            if always_draws:
                self._add_warning("All matches result in draws - suspicious pattern")
    
    def _check_season_completeness(self, df: pd.DataFrame, season: str, context: str = ""):
        ...
        expected = expected_matches_per_team.get(n_teams, n_teams * 2 - 2)  # Default: round robin home/away
        ...
        self._add_info(f"Season completeness: {n_matches}/{expected_total} matches ({completeness:.1%})")
        
        if completeness < 0.8:
            self._add_warning(f"Season appears incomplete: only {completeness:.1%} of expected matches")
        elif completeness > 1.2:
            self._add_warning(f"More matches than expected: {completeness:.1%} of typical season")
    
    def validate_historical_coverage(self, data_sources: Dict[str, pd.DataFrame], 
                                   required_seasons: List[str] = None) -> Dict[str, Any]:
        """
        ...
        if required_seasons:
            ...
            coverage_report['season_coverage'] = {
                ...
                'coverage_rate': len(all_seasons & set(required_seasons)) / len(required_seasons)
            }
        
        self.validation_report['coverage_analysis'] = coverage_report
        return self.validation_report
    
    def cross_validate_sources(self, source1: pd.DataFrame, source2: pd.DataFrame,
                             source1_name: str = "Source1", source2_name: str = "Source2") -> Dict[str, Any]:
        """
        ...
        if not all(col in source1.columns and col in source2.columns for col in common_cols):
        ...
        try:
            merged = source1.merge(source2, on=merge_cols, how='inner', suffixes=('_s1', '_s2'))
            
            if merged.empty:
                self._add_warning(f"No common matches found between {source1_name} and {source2_name}")
                return self.validation_report
            ...
            # Compare goals
            goal_mismatches = (
                (merged['goals_home_s1'] != merged['goals_home_s2']) |
            ...
            self.validation_report['cross_validation'] = {
                ...
                'mismatch_rate': goal_mismatches.sum() / len(merged) if len(merged) > 0 else 0
            }
    ...
    def generate_summary_report(self) -> str:
        ...
        if error_count == 0 and warning_count == 0:
            ...
            report_lines.append(f"❌ DATA VALIDATION FAILED - {error_count} errors, {warning_count} warnings")
        
        return "\n".join(report_lines)


# Convenience functions
def validate_fixtures(df: pd.DataFrame, competition: str = "", season: str = "", 
...
def cross_validate_sources(source1: pd.DataFrame, source2: pd.DataFrame,
    ...
    """
    validator = DataQualityValidator(strict_mode=strict_mode)
    return validator.cross_validate_sources(source1, source2, source1_name, source2_name)